...
class ORMFixture:
    ...
    def convert_group_to_model(self, groups):
        def convert(group):
            return Group(id=str(group.id), header=group.header, footer=group.footer, name=group.name)
    ...
    def convert_contact_to_model(self, contacts):
        def convert(contact):
            return Address(id=str(contact.id), firstname=contact.firstname, lastname=contact.lastname)
